...
export default function Settings() {
  ...
  const handleGenerateSampleMonth = async () => {
    await generateSampleMonth();
    setNotification({
      message: "Sample month generated successfully",
      severity: "success",
    });
  };
    ...
    <Box sx={{ maxWidth: 600, mx: "auto" }}>
      ...
      <Paper sx={{ p: 3 }}>
        ...
        <Typography variant="body2" color="text.secondary">
          Export your data to create a backup, or import a previously exported
          file. Note that importing will replace all existing data.
        </Typography>
      </Paper>
      <Divider sx={{ my: 4 }} />
      <Paper sx={{ p: 3 }}>
      ...
      <Dialog open={importDialog} onClose={() => setImportDialog(false)}>
        ...
        <DialogContent>
          <DialogContentText>
            This will replace all existing data with the contents of{" "}
            {selectedFile?.name}. This action cannot be undone. Are you sure you
            want to continue?
          </DialogContentText>
        </DialogContent>
        <DialogActions>
